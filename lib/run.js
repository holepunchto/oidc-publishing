...
module.exports = async function run(command, args, opts = {}) {
  ...
  const inherited = spawnOpts.stdio === 'inherit'
  ...
  return new Promise((resolve, reject) => {
    job.on('close', (code) => {
      if (code === null || code !== 0) {
        ...
        if (!inherited) opts.cause = Buffer.concat(err).toString().trim()

        return reject(new Error(`Command '${command} ${args.join(' ')}' failed`, opts))
      }

      resolve(inherited ? '' : Buffer.concat(out).toString().trim())
    })
  })
}
